        ...
        public function insertar_metodo_pago($id_caja, $metodo_pago, $cantidad_pagar){
            ...
            if($insertar_metodo_pago->execute()){
                ...
                if($buscar_metodo->execute()){
                    ...
                    // ✅ CORREGIDO: ELIMINAR LAS LÍNEAS DUPLICADAS QUE INCLUYEN EL ARCHIVO
                    if(!empty($metodo_info['incremento']) || !empty($metodo_info['decremento'])){
                        ...
                        if($buscar_caja->execute()){
                            $resultado_caja = $buscar_caja->get_result();
